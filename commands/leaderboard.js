...
module.exports = {
        ...
        data: new SlashCommandBuilder()
                .setName("leaderboard")
                .setDescription(
                        "View the immersion leaderboard based on points.",
                )
                ...
                .addStringOption((option) =>
                        option
                                .setName("media_type")
                                .setDescription(
                                        "Select media type for the leaderboard (or 'all' for all).",
                                )
                                .setRequired(true)
                                .addChoices(
                ...
                .addIntegerOption((option) =>
                        option
                                .setName("month")
                                .setDescription(
                                        "Select a specific month for the monthly leaderboard (optional).",
                                )
                                .setRequired(false)
                                .addChoices(
                ...
                .addIntegerOption(
                        (option) =>
                                option
                                        .setName("year")
                                        .setDescription(
                                                "Select a specific year for the monthly/yearly leaderboard (optional).",
                                        )
                                        .setRequired(false)
        ...
        async execute(interaction) {
                ...
                const mediaTypeFilter =
                        interaction.options.getString("media_type");
                // --- PERUBAHAN DIMULAI DI SINI ---
                ...
                if (!timestampFilter || !mediaTypeFilter) {
                        return await interaction.editReply(
                                "Please select both a time period and a media type for the leaderboard.",
                        );
                }
                ...
                switch (timestampFilter) {
                        ...
                        case "monthly": {
                                const year =
                                        specifiedYear || now.getUTCFullYear();
                ...
                try {
                        ...
                        if (userDocs.empty) {
                                return await interaction.editReply(
                                        "🚫 No immersion data recorded yet.",
                                );
                        }
                        ...
                        for (const userDoc of userDocs.docs) {
                                ...
                                if (timestampFilter === "all_time") {
                                        ...
                                        logsSnapshot.forEach((logDoc) => {
                                                ...
                                                const logMediaType =
                                                        logData.activity.type;
                                                const logAmount =
                                                        logData.activity.amount;
                        ...
                        const sortedLeaderboard = Object.values(
                                leaderboard,
                        ).sort((a, b) => b.points - a.points);

                        if (sortedLeaderboard.length === 0) {
                                return await interaction.editReply(
                                        `🚫 No immersion data found for the **${titlePeriod}** period and **${mediaTypeLabelMap[mediaTypeFilter]}** media type.`,
                                );
                        }
                        ...
                        const embed = new EmbedBuilder()
                                ...
                                .setTitle(
                                        `🏆 Immersion Leaderboard - ${titlePeriod} (${mediaTypeLabelMap[mediaTypeFilter]})`,
                                )
                                .setDescription(
                                        "Here's the list of top immersionists:",
                                )
                                .setTimestamp()
                                .setFooter({
                                        text: `Generated by ${interaction.client.user.username}`,
